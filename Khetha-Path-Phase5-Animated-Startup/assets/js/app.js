...
// PWA install prompt: allows supported browsers to install Khetha Path like an app.
...
document.addEventListener("click", async (event) => {
  if (event.target && event.target.id === "installBtn" && deferredInstallPrompt) {
...
// Store the learner's basic journey locally so the mobile experience can work
// without a network connection. Production should use encrypted/local secure storage
// plus an authenticated sync API.
const journeyForm = document.getElementById("startForm");
if (journeyForm) {
  journeyForm.addEventListener("submit", () => {
    const name = document.getElementById("name")?.value || "";
    const subjects = [...document.querySelectorAll('input[name="subjects[]"]:checked')].map(x => x.value);
    const interest = document.querySelector('input[name="interest"]')?.value || "";
    const grade = document.querySelector('select[name="grade"]')?.value || "";
    localStorage.setItem("khethaJourney", JSON.stringify({
      name, subjects, interest, grade, savedAt: new Date().toISOString()
    }));
  });
}
